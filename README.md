<div align="center">

<img src="https://s6.imgcdn.dev/Yo1msH.png" alt="AW:U Logo" width="10%" height="10%">

# Adventure World: Unbound

[![Official Website][img-website]][url-website]
[![Support and Contribute][img-contribute]][url-contribute]
[![Read the Wiki][img-wiki]][url-wiki]

[![Discord][img-discord]][url-discord]
[![GitHub][img-youtube]][url-youtube]

A Minecraft Adventure Modpack with tons of cool features.

</div>

## Download

[![Modrinth][img-modrinth]][url-modrinth]
> We are still working on the CurseForge page, so use the Modrinth link to download the Modpack.

## Credits

- **Modpack Making**
    
    Thanks to:
    
    - wydaedoug - Creator
    - wildzizo  - Ideas, Support
    - blobboj   - Ideas
- **Mods**
    - Alternate Current — Space Walker
    - AppleSkin — squeek502
    - Architecture API — architecture
    - BadOptimizations — thosea
    - Better Block Entities — cseden
    - Better Combat — ZsoltMolnarrr
    - BetterF3 — TreyRuffy
    - Better Statistics Screen — TheCSDev
    - Biomes O' Plenty — Forstride
    - Blooming Biosphere — hhumanoid
    - Bookshelf Inspector — lukasabbe
    - Cardinal Components API — Ladysnake
    - Chat Animation [Smooth Chat] — Ezzenix
    - Cherished Worlds — TheIllusiveC4
    - Client Sort — TerminalMC
    - Cloth Config API — shedaniel
    - Clumps — jaredlll08
    - Collective — Serilum
    - Controlling — jaredlll08
    - Crops Love Rain — ThePoultryMan
    - Custom Glint — cutebow
    - Cut Through — Fuzs
    - DeCubed Dungeons (Formerly Dungeons+) — technodono
    - Easy Anvils — Fuzs
    - Easy Magic — Fuzs
    - Easy Shulker Boxes — Fuzs
    - Echo Chest — Fuzs
    - Enchanted Fishing Line — Punchy Guys Studios
    - Enchanting Infuser — Fuzs
    - Entity Culling — tr7zw
    - Essential Mod — SparkUniverse
    - Fabric API — modmuss50
    - Fabric Language Kotlin — modmuss50
    - Fast IP Ping — fallen-breath
    - Fast Item Frames — Fuzs
    - FastQuit — contaria
    - FerriteCore — malte0811
    - Forge Config API Port — Fuzs
    - Freecam — hashalite
    - Fzzy Config — fzzyhmstrs
    - Geckolib — Gecko
    - Geophilic — bebebea_loste
    - Hopo Better Ruined Portals — Hoponopono
    - Illager Invasion — Fuzs
    - ImmediatelyFast — RaphiMC
    - Immersive Optimization — Luke100000
    - Inventory Interactions — LopyMine
    - Inventory Particles — LopyMine
    - Iris Shaders — coderbot
    - JamLib — Jam Core
    - Krypton — astei
    - LambDynamicLights - Dynamic Lights — LambdAurora
    - Leaves Be Gone — Fuzs
    - Litematica — masa
    - Lithium — CaffeineMC
    - Lithostitched — Apollo
    - MaLiLib — masa
    - Dungeons Dimensions: Nether — Redupro
    - Mindful Darkness — Fuzs
    - Mod Menu — Terraformers
    - More Mobs — Tschipcraft
    - More Chests Variants (MCV) — LieOn Studios
    - MossyLib — LopyMine
    - Mouse Tweaks — YaLTeR
    - Neo Enchant+ — Hardel-DW
    - Nether Chested — Fuzs
    - NoisiumForked — Coredex
    - No Resource Pack Warnings — Space Walker
    - Not Enough Animations — tr7zw
    - Nullscape — Stardust Labs
    - Numeral Ping — TheKodeToad
    - Overflowing Bars — Fuzs
    - Packed Packs — fishstiz
    - Packet Fixer — TonimatasDEV
    - Paginated Advancements & Custom Frames — DaFuqs
    - Pets Locator — Tomokisan
    - Pickup Notifications — Roundaround
    - Picture Mode — icanttellyou
    - Ping View — karlsson1000
    - Text Placeholder API — Patbox
    - Plasmo Voice — kpids
    - Player Animation Library — ZigyTheBird
    - Pocket Dimensions — MavLeague
    - Podium — AndreasMelone_
    - Punchy! x Tiny Takeover — Punchy Guys Studios
    - Punchy! — Punchy Guys Studios
    - Puzzles Lib — Fuzs
    - Reese's Sodium Options — FlashyReese
    - Resourceful Lib — Team Resourceful
    - Resource Pack Overrides — Fuzs
    - Resourcify — DeDiamondPro
    - RightClickHarvest — Jam Core
    - Scorchful — TheDeathlyCow
    - Searchables — jaredlll08
    - Shield Statuses — Walksy
    - Smooth Gui — Ezzenix
    - SmoothHud — treegfx
    - Sodium Extra — FlashyReese
    - Sodium — CaffeineMC
    - Softer Hay Bales — Serilum
    - Soft Imprints — nNined
    - Status Effect Bars — A5b84
    - Stoneworks — Fuzs
    - Streams Reflowing — nice.john
    - Subtle Effects — MincraftEinstein
    - Subtly Dungeons — Meander Studios
    - Sword Blocking Mechanics — Fuzs
    - TCDCommons API — TheCSDev
    - TerraBlender — Adubbz
    - Terralith — Stardust Labs
    - Terralith: Extra Recipes — Stardust Labs
    - Terralith: ReStoned — Stardust Labs
    - Custom Biome Saplings — limesplatus
    - Thermoo — TheDeathlyCow
    - True Ending - Ender Dragon Overhaul — limesplatus
    - WalksyLib — Walksy
    - wWaypoints — wSmoothie
    - YetAnotherConfigLib (YACL) — isxander
    - YUNG's API — Refresh Studios
    - YUNG's Better Caves — Refresh Studios
    - YUNG's Better Desert Temples — Refresh Studios
    - YUNG's Better Dungeons — Refresh Studios
    - YUNG's Better End Island — Refresh Studios
    - YUNG's Better Jungle Temples — Refresh Studios
    - YUNG's Better Mineshafts — Refresh Studios
    - YUNG's Better Nether Fortresses — Refresh Studios
    - YUNG's Better Ocean Monuments — Refresh Studios
    - YUNG's Better Strongholds — Refresh Studios
    - YUNG's Better Witch Huts — Refresh Studios
    - YUNG's Bridges — Refresh Studios
    - YUNG's Cave Biomes — Refresh Studios
    - Zoomify (Zoom) — isxander
- **Resource Packs**
    - Adventure Panorama — W_aves
    - Adventurer's GUI — RedCarlos
    - Arcle's Medieval Containers — ArcleBreeze
    - Aures - Medieval Style Lootr — BlackAures
    - Chat Reporting Helper — Fabulously Optimized
    - Epic Adventures — tobyplowy
    - Excalibur | Fallen Adventurers Support — Guardian_50
    - Fullbright UB — worldresourcepack
    - Low On Fire — Haikis
    - Medieval Loading Screens — SwaixYT
    - Motschen's Better Leaves — Motschen
    - Optimum Realism — AtmosphereOfTech
    - RealisticLite Shader — ratanjunior334
    - Translations for Sodium — robotkoer
    - Whimscape — kavast

---

## Feedback

**Email:** [wydaedoug@gmail.com](mailto:wydaedoug@gmail.com)

**Location:** Remote

<!-- Images -->
[img-discord]: <https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=Discord&logoColor=white&link=discord.gg%2Fr4us6fxYyA>
[img-youtube]: <https://img.shields.io/badge/Youtube-FF0000?style=for-the-badge&logo=Youtube&logoColor=white&link=www.youtube.com%2F%40wydaedoug>
[img-modrinth]: <https://img.shields.io/badge/Modrinth-%2300af5c?style=for-the-badge&logo=Modrinth&logoColor=white&link=https%3A%2F%2Fmodrinth.com%2Fproject%2F1ob5lhR1>
[img-options]: <https://img.shields.io/badge/Changed_Options-blue?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%3D>
[img-contribute]: <https://img.shields.io/badge/Support_and_Contribute-red?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%>
[img-translate]: <https://img.shields.io/badge/Help_us_Translate-orange?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BPC9zdmc%2B>
[img-website]: <https://img.shields.io/badge/Website-FF9505?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%>
[img-wiki]: <https://img.shields.io/badge/Read_the_Wiki-423E35?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%3D%3D>

<!-- URLs -->
[url-app-modrinth]: <https://modrinth.com/modpack/adventure-world-unbound>
[url-discord]: <https://dc.gg/advntrwrld>
[url-youtube]: <https://youtube.com/@wydaedoug>
[url-modrinth]: <https://modrinth.com/project/1ob5lhR1>
[url-contribute]: <https://ko-fi.com/wydae>
[url-website]: <https://flyn.co/adventureworldunbound>
[url-wiki]: <https://notion-url-shortener.vercel.app/adventureworldunbound>
